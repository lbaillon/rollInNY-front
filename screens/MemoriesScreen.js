...
export default function MemoriesScreen({ route, navigation }) {
    ...
    const fetchPictures = async () => {
        try {
            const response = await fetch(
                `https://roll-in-ny-back.vercel.app/favorites/pictures/${user.token}/${selectedPlace.id}`
            );
            ...
            console.error(
                "❌ (Memories Screen) : Error to fetch all pictures",
                err
            );
            setLoading(false);
        }
    ...
    const handlePostReview = () => {
        ...
        if (newReviewText === "") {
            ...
            try {
                fetch(
                    `https://roll-in-ny-back.vercel.app/reviews/${user.token}/${selectedPlace.id}`,
                    {
                        method: "POST",
                ...
                console.error(
                    "❌ (Memories Screen): Error in connection to database",
                    err
                );
            }
        }
    ...
    const handleFilePick = async () => {
        try {
            ...
            if (status !== "granted") {
                Alert.alert(
                    "Permission denied",
                    "You must allow access to your photo library to upload a picture."
                );
                return;
            }
            ...
            if (!result.canceled && result.assets.length > 0) {
                for (const asset of result.assets) {
                    try {
                        ...
                        const response = await fetch(
                            "https://roll-in-ny-back.vercel.app/favorites/pictures",
                            {
                                method: "POST",
                                body: formData,
                            }
                        ...
                        if (data.url) {
                            ...
                            Alert.alert(
                                ...
                                "An error occurred while uploading a picture."
                            );
                        }
                    } catch (error) {
                        console.error(
                            "❌ (Memories Screen): Error in photo upload",
                            error
                        );
                    }
                }
            ...
            console.error(
                "❌ (Memories Screen): Error in picture selection",
                error
            );
            Alert.alert(
                "Photo selection failed",
                "An error occurred while selecting a picture."
            );
        }
    };
