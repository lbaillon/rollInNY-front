...
const manWalking =
    "https://res.cloudinary.com/dtkac5fah/image/upload/v1734427710/appIcons/l0misyhittkq0v7qabx3.webp";
const moviePlace =
    "https://res.cloudinary.com/dtkac5fah/image/upload/v1734427585/appIcons/oi2ry9sz9uojhzasypfv.webp";
const errorImage =
...
export default function FavoriteScreen() {
    ...
    useEffect(() => {
        ...
        try {
            fetch(
                `https://roll-in-ny-back.vercel.app/favorites/places/${user.token}`
            )
                ...
                .catch((err) => {
                    console.error(
                        "❌ (Favorite Screen): Error in fetch liked places",
                        err
                    );
                });
        } catch (err) {
            console.error(
                "❌ (Favorite Screen): Error in database connection",
                err
            );
        }
        ...
        content = (
            <View style={styles.errorBox}>
                ...
                <Text style={styles.textError}>
                    No favorite places at the moment
                </Text>
            </View>
        );
    } else {
    ...
    const goToMap = () => {
        if (placesLikedList && checkedStates.includes(true)) {
            const selectedPlaces = placesLikedList.filter(
                (_, i) => checkedStates[i]
            );
            ...
            const sortedPlaces = selectedPlaces.sort(
                (a, b) =>
                    calculateDistance(originCoords, a.coords) -
                    calculateDistance(originCoords, b.coords)
            );
            ...
            const destination = `${
                sortedPlaces[sortedPlaces.length - 1].coords.lat
            ...
            Alert.alert(
                "No place selected",
                "Please select at least one place to plan your day."
            );
        }
    };
