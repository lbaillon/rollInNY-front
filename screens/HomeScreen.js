...
export default function HomeScreen({ navigation }) {
    ...
    const [currentPosition, setCurrentPosition] = useState({
        latitude: 40.7649861,
        longitude: -73.9680353,
    }); // Initialisation des coordonnées de localisation de l'utilisateur à la position de central park
    ...
    useEffect(() => {
        ...
        try {
                ...
                mapRef.current.animateToRegion(
                    {
                        latitude:
                            placeCoords.lat +
                            (Platform.OS === "ios" ? 0.0058 : 0), // Ajustez le décalage selon vos besoins
            ...
            console.error(
                "❌ (Home Screen): Error in connection to database",
                err
            );
        }
    }, [placeCoords]);
