...
export default function PlaceCard({
    ...
    useEffect(() => {
        (async () => {
            try {
                fetch(
                    `https://roll-in-ny-back.vercel.app/users/isLiked/${user.token}/${id}`
                )
                    ...
                    .catch((err) =>
                        console.error(
                            "❌ (PlaceCard) Error in set like status",
                            err
                        )
                    );
            } catch (err) {
                console.error(
                    "❌ (PlaceCard) Error in connection database",
                    err
                );
            }
        })();
    ...
    useEffect(() => {
        try {
            fetch(`https://roll-in-ny-back.vercel.app/reviews/${placeInfo.id}`)
                .then((response) => response.json())
                .then((data) => {
                    setReviewsTable(data.reviews);
                })
                .catch((err) =>
                    console.error("❌ (PlaceCard) Error in fetch reviews", err)
                );
        } catch (err) {
    ...
    const handleLike = () => {
        ...
        try {
            fetch(
                `https://roll-in-ny-back.vercel.app/users/likePlace/${user.token}/${id}`,
                {
                    method: "PUT",
                }
            )
                ...
                .catch((err) =>
                    console.error(
                        "❌ (PlaceCard) Error in change liked state",
                        err
                    )
                );
        } catch (err) {
    ...
    return (
        <View style={styles.container}>
            <Pressable
                onPress={() =>
                    nav.getState().routes[nav.getState().index].name ===
                    "Memories"
                        ? null
                        : togglePopup()
                }
